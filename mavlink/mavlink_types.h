 ...
 * native type as well.
 */
MAVPACKED(typedef struct param_union {
    union {
        float param_float;
        int32_t param_int32;
 ...
 */
MAVPACKED(typedef struct param_union_extended {
    union {
        struct
        {
            uint8_t is_double : 1;
 ...
 */
MAVPACKED(typedef struct __mavlink_system {
    uint8_t sysid;  ///< Used by the MAVLink message_xx_send() convenience function
    uint8_t compid; ///< Used by the MAVLink message_xx_send() convenience function
})
mavlink_system_t;

MAVPACKED(typedef struct __mavlink_message {
...
MAVPACKED(typedef struct __mavlink_attitude_t {
...
MAVPACKED(typedef struct __mavlink_gps_raw_int_t {
    ...
    uint16_t yaw; /*< [cdeg] Yaw in earth frame from north. Use 0 if this GPS does not provide yaw. Use UINT16_MAX if this GPS is configured to provide yaw and is currently unable to provide
...
MAVPACKED(typedef struct __mavlink_global_position_int_t {
    uint32_t time_boot_ms; /*< [ms] Timestamp (time since system boot).*/
...
MAVPACKED(typedef struct __mavlink_extended_sys_state_t {
...
MAVPACKED(typedef struct __mavlink_command_long_t {
